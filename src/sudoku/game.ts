...
export const newGame = ({
  ...
  for (let row = 0; row < degree; row++)
    for (let col = 0; col < degree; col++) board[row][col].solution = solution[row][col].value
...
export const checkConflicts = (game: Game): void => {
  for (const row of game.board)
    for (const cell of row) {
      ...
      const conflicts = findConflicts(game.board, cell.location, cell.value, game.degree)
